    ...
    public class LcMsPeakMatrix
    {
        ...
        public double GetMs1EvidenceScore(int ms2ScanNum, double targetMass, int charge)
        {
            ...
            for (var i = 0; i <= maxSearchScans; i++)
            {
                for (var j = 0; j < 2; j++)
                {
                    ...
                    if (pearsonCorrelationCoefficient > 0.7)
                    {
                        return pearsonCorrelationCoefficient;
                    }

                    bcDistances.Add(bcDist);
                    correlations.Add(pearsonCorrelationCoefficient);
                }
            }
        ...
        private IList<LcMsPeakCluster> GetLcMs1PeakClusters(int binNumber)
        {
            ...
            foreach (var seed in _seedEnvelopes.OrderBy(s => s.Key).Select(s => s.Value))
            {
                ...
                var summedCorr = _featureMatrix[row][col].CorrelationCoefficient;
                ...
                while (neighbors.Count > 0)
                {
                    ...
                    var currentCol = ms1ScanNumToIndex[cell.ScanNum];
                    ...
                    for (var k = 0; k < 5; k++)
                    {
                        ...
                        for (var i = minRw; i <= maxRw; i++)
                        {
                            ...
                            if (_featureMatrix[i][j].DivergenceDist < 0.02 ||
                                _featureMatrix[i][j].CorrelationCoefficient > 0.7 ||
                ...
                for (var j = minCol; j <= maxCol; j++)
                {
                    ...
                    if (reCollectAllPeaks || !_featureMatrix[i][j].Exist || Math.Abs(_featureMatrix[i][j].AccurateMass - targetMass) > massTol)
                    {
                        ...
                        if (peaks.Count(p => p != null) > 0)
                        {
                            ...
                            _featureMatrix[i][j].CorrelationCoefficient = _theoreticalEnvelope.GetPearsonCorrelation(peaks);
        ...
        private Tuple<int, int, int, int> DetermineFeatureRange(double targetMass, int seedRow, int seedMinCol, int seedMaxCol)
        {
            ...
            // for each charge state, find and evaluate optimal summed envelope
            // find column range
        ...
        private double[] GetSummedEnvelopeAtCharge(double targetMass, int row, int minCol, int maxCol)
        {
            ...
            const double goodEnoughCorrCoefficient = 0.7;
            ...
            for (var col = seedCol; col < NColumns; col++)
            {
                ...
                if (tempBcDist < summedBcDist ||
                    tempCorr > summedCorr ||
                    _featureMatrix[row][col].DivergenceDist < goodEnoughBcDistance ||
                    _featureMatrix[row][col].CorrelationCoefficient > goodEnoughCorrCoefficient)
                {
        ...
        private Tuple<int, int> GetElutionWindow(int refCol, double halfWindowSize, int minScanCount = 5)
        {
            ...
            maxCol = Math.Max(maxCol, ShiftColumnByElutionTime(refCol, halfWindowSize));
            minCol = Math.Max(minCol, ShiftColumnByElutionTime(refCol, -halfWindowSize));
            return new Tuple<int, int>(minCol, maxCol);
        }

        private int ShiftColumnByElutionTime(int refCol, double elutionShift)
        {
            var ms1ScanNums = Run.GetMs1ScanVector();
        ...
        {
            internal double CorrelationCoefficient;
