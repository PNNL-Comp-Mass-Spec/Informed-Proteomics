...
namespace InformedProteomics.TopDown.TagBasedSearch
{
    public class ScanBasedTagSearchEngine
    {
        ...
        public IEnumerable<TagSequenceMatch> RunSearch(int ms2ScanNum)
        {
            ...
            {
                scorer = (_ms2ScorerFactory != null) ? _ms2ScorerFactory.GetScorer(ms2ScanNum) : new CorrMatchedPeakCounter(spec, _tolerance, _minProductIonCharge, _maxProductIonCharge);
            }
            else
            {
                scorer = (_ms2ScorerFactory != null) ? _ms2ScorerFactory.GetMs2Scorer(ms2ScanNum) : new CorrMatchedPeakCounter(spec, _tolerance, _minProductIonCharge, _maxProductIonCharge);
            }
